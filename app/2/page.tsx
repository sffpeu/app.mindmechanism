...
function NodesPageContent() {
  ...
  // Read originalDuration from URL on mount so clock-page progress ring is correct when continuing a session
  ...
  // Fetch session when continuing — use same session.duration as dashboard so progress ring matches recent sessions
  useEffect(() => {
    if (!sessionId) return
    ...
    <ProtectedRoute>
      ...
      <div className="h-full overflow-x-hidden flex flex-col bg-gray-50 dark:bg-black/95 min-h-0">
        ...
        <div className={cn("fixed bottom-4 left-4 z-50 flex items-center gap-2 transition-opacity duration-700", isIdle && "opacity-0 pointer-events-none")}>
          ...
          <DraggableClockPanel open={isInfoOpen} onClose={() => setIsInfoOpen(false)} clockHex={clockHex} clockIndex={2} instanceId="info-2" panelWidth="300px">
          ...
          <div className="relative w-[82vw] h-[82vw] max-w-[615px] max-h-[615px] overflow-visible" onPointerMove={resetNodeNumberTimer} onPointerDown={resetNodeNumberTimer}>
            ...
            <div className="absolute inset-0 rounded-full overflow-hidden">
              ...
              >
                ...
                >
                  <Image 
                    src={colourMode === 'colour' ? '/clock_3_colour.svg' : clock2.imageUrl}
                    alt="Clock Face 2"
                    layout="fill"
                    objectFit="cover"
                    ...
                    loading="eager"
                  />
                  <ClockWheelFaceOverlay clockId={2} />
                </div>
              </motion.div>
              <ClockVideoOverlay clockId={2} />
              {showCeremony && (
                <MandalaCeremony
                  clockHex={clockHex}
            ...
            >
              <div className="absolute inset-0 pointer-events-none" style={{ transform: `rotate(${clock2.imageOrientation}deg)` }}>
