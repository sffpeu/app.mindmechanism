...
function NodesPageContent() {
  ...
  const getWordContainerStyle = (angle: number, isSelected: boolean) => {
    ...
    const counterRotation = -rotation - clock8.imageOrientation
    const offsetPx = 28
  ...
  const defaultWords = DEFAULT_WORDS_BY_CLOCK[8] ?? []
  const getFocusNodeStyle = (index: number, isSelected: boolean) => {
    const color = '#56c1ff' // Dot menu hover color for clock 8 (light blue)
    return {
  ...
  const clockColor = hexToRgb('#56c1ff') // Dot menu color for clock 8 (light blue)
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2 pb-3 border-b border-black/10 dark:border-white/10 text-center">
                      ...
                      <p className="text-xs text-black/60 dark:text-white/60">
                        {clock8.startDateTime.getDate().toString().padStart(2, '0')}.{(clock8.startDateTime.getMonth() + 1).toString().padStart(2, '0')}.{clock8.startDateTime.getFullYear()}
                      </p>
