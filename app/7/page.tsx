...
function NodesPageContent() {
  ...
  const getWordContainerStyle = (angle: number, isSelected: boolean) => {
    ...
    const counterRotation = -rotation - clock7.imageOrientation
    const offsetPx = 28
  ...
  const defaultWords = DEFAULT_WORDS_BY_CLOCK[7] ?? []
  const getFocusNodeStyle = (index: number, isSelected: boolean) => {
    const color = '#ee5fa7' // Dot menu hover color for clock 7 (pink)
    return {
  ...
  const clockColor = hexToRgb('#ee5fa7') // Dot menu color for clock 7 (pink)
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2 pb-3 border-b border-black/10 dark:border-white/10 text-center">
                      ...
                      <p className="text-xs text-black/60 dark:text-white/60">
                        {clock7.startDateTime.getDate().toString().padStart(2, '0')}.{(clock7.startDateTime.getMonth() + 1).toString().padStart(2, '0')}.{clock7.startDateTime.getFullYear()}
                      </p>
