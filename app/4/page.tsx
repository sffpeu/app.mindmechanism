...
// Clock 1–9 hex palette; this page uses index 4 (clock 5)
const CLOCK_INDEX = 4
...
function NodesPageContent() {
  ...
  const focusNodes = clock4.focusNodes
  const startingDegree = clock4.startingDegree
  const rotationTime = clock4.rotationTime
  const rotationDirection = clock4.rotationDirection
  ...
  const defaultWords = DEFAULT_WORDS_BY_CLOCK[4] ?? []
  ...
  return (
    <ProtectedRoute>
      <ClockBreathingTone clockIndex={4} />
      <div className="h-full overflow-x-hidden flex flex-col bg-gray-50 dark:bg-black/95 min-h-0">
        ...
        <div className={cn("fixed top-4 right-14 z-[200] transition-opacity duration-700", isIdle && "opacity-0 pointer-events-none")}>
          ...
          <DraggableClockPanel open={isDropdownOpen} onClose={() => setIsDropdownOpen(false)} clockHex={clockHex} clockIndex={4}>
              <ClockPanelRow>
        ...
        <div className={cn("fixed bottom-4 left-4 z-50 flex items-center gap-2 transition-opacity duration-700", isIdle && "opacity-0 pointer-events-none")}>
          ...
          <DraggableClockPanel open={isInfoOpen} onClose={() => setIsInfoOpen(false)} clockHex={clockHex} clockIndex={4} instanceId="info-4" panelWidth="300px">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2 pb-3 border-b border-black/10 dark:border-white/10 text-center">
                      ...
                      <p className="text-xs text-black/60 dark:text-white/60">
                        {clock4.startDateTime.getDate().toString().padStart(2, '0')}.{(clock4.startDateTime.getMonth() + 1).toString().padStart(2, '0')}.{clock4.startDateTime.getFullYear()}
                      </p>
                      <p className="text-xs text-black/60 dark:text-white/60 line-clamp-2">
                        The unique signature of clock 5, established for psychological mapping and somatic resonance.
                      </p>
                    </div>
        ...
        <div className="flex-grow flex items-center justify-center min-h-0 overflow-visible py-8">
          <div className="relative w-[82vw] h-[82vw] max-w-[615px] max-h-[615px] overflow-visible" onPointerMove={resetNodeNumberTimer} onPointerDown={resetNodeNumberTimer}>
            ...
            {showSatellites && (
              ...
              >
                <ClockPageSatelliteLayer
                  ...
                  startDateTime={clock4.startDateTime}
                />
              </motion.div>
            )}
            ...
            <div className="absolute inset-0 rounded-full overflow-hidden">
              ...
              >
                <div
                  className="absolute inset-0"
                  style={{
                    transform: `translate(${clock4.imageX}%, ${clock4.imageY}%) rotate(${clock4.imageOrientation}deg) scale(${clock4.imageScale})`,
                    willChange: 'transform',
                    transformOrigin: 'center',
                  }}
                >
                  <Image 
                    src={colourMode === 'colour' ? '/clock_5_colour.svg' : clock4.imageUrl}
                    alt="Clock Face 4"
                    layout="fill"
                    objectFit="cover"
                    ...
                    loading="eager"
                  />
                  <ClockWheelFaceOverlay clockId={4} />
                </div>
              </motion.div>
              <ClockVideoOverlay clockId={4} />
              {showCeremony && (
                <MandalaCeremony
                  clockHex={clockHex}
            ...
            >
              <div className="absolute inset-0 pointer-events-none" style={{ transform: `rotate(${clock4.imageOrientation}deg)` }}>
