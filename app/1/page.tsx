...
function NodesPageContent() {
  ...
  const handleCardDragStart = useCallback((e: React.MouseEvent) => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      setCardPosition({ x: dragRef.current.startLeft + e.clientX - dragRef.current.startX, y: dragRef.current.startTop + e.clientY - dragRef.current.startY })
    }
    const onUp = () => {
  ...
  const defaultWords = DEFAULT_WORDS_BY_CLOCK[1] ?? []
  ...
  return (
    <ProtectedRoute>
      <ClockBreathingTone clockIndex={1} />
      <div className="h-full overflow-x-hidden flex flex-col bg-gray-50 dark:bg-black/95 min-h-0">
        ...
        <div className={cn("fixed top-4 right-14 z-[200] transition-opacity duration-700", isIdle && "opacity-0 pointer-events-none")}>
          ...
          <DraggableClockPanel open={isDropdownOpen} onClose={() => setIsDropdownOpen(false)} clockHex={clockHex} clockIndex={1}>
              <ClockPanelRow>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2 pb-3 border-b border-black/10 dark:border-white/10 text-center">
                      ...
                      <p className="text-xs text-black/60 dark:text-white/60">
                        {clock1.startDateTime.getDate().toString().padStart(2, '0')}.{(clock1.startDateTime.getMonth() + 1).toString().padStart(2, '0')}.{clock1.startDateTime.getFullYear()}
                      </p>
                      <p className="text-xs text-black/60 dark:text-white/60 line-clamp-2">
                        The unique signature of clock 2, established for psychological mapping and somatic resonance.
                      </p>
                    </div>
          ...
          <div className="relative w-[82vw] h-[82vw] max-w-[615px] max-h-[615px] overflow-visible" onPointerMove={resetNodeNumberTimer} onPointerDown={resetNodeNumberTimer}>
            ...
            {showSatellites && (
              ...
              >
                <ClockPageSatelliteLayer
                  ...
                  startDateTime={clock1.startDateTime}
                />
              </motion.div>
            )}
            ...
            <div className="absolute inset-0 rounded-full overflow-hidden">
              ...
              >
                ...
                >
                  <Image 
                    src={colourMode === 'colour' ? '/clock_2_colour.svg' : clock1.imageUrl}
                    alt="Clock Face 1"
                    layout="fill"
                    objectFit="cover"
                    ...
                    loading="eager"
                  />
                  <ClockWheelFaceOverlay clockId={1} />
                </div>
              </motion.div>
            ...
            >
              <div className="absolute inset-0 pointer-events-none" style={{ transform: `rotate(${clock1.imageOrientation}deg)` }}>
