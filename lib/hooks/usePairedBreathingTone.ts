...
const MAX_GAIN_SYNTH = 0.09
const MAX_GAIN_DRONE = 0.075

/**
 * Two wheels: sine or looped drones, same breathing envelope, respects Sound settings.
 */
export function usePairedBreathingTone(
  ...
  useEffect(() => {
    ...
    const startDrone = async () => {
      try {
        ...
        console.warn('[PairedBreathingTone] Drone load failed, using sine', e)
  ...
  }, [clockIndexA, clockIndexB, toneMode, settingsHydrated])
}
