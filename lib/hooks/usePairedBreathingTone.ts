...
/** Slightly lower per-oscillator ceiling so two tones together don't clip */
const MAX_GAIN = 0.09
...
/**
 * Plays two sine tones simultaneously (one per wheel) in a single AudioContext.
 * Both tones breathe in phase on the 60-second envelope. Muted state is
 * reactive via ref — toggling mute does not restart the AudioContext.
 */
export function usePairedBreathingTone(
  clockIndexA: number,
  clockIndexB: number,
  muted: boolean
) {
